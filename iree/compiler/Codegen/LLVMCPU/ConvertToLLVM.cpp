...
class HALDispatchABI {
 public:
  ...
  // Matches IREE_HAL_PROCESSOR_DATA_V0_CAPACITY.
  static constexpr int ProcessorDataCapacity = 4;
  ...
  static LLVM::LLVMStructType getProcessorType(
    ...
    // uint64_t data[IREE_HAL_PROCESSOR_DATA_V0_CAPACITY];
  ...
  // Loads the processor flags. Only the lower 56 bits are usable.
  Value loadProcessorFlags(Location loc, OpBuilder &builder) {
    return loadFieldValue(loc, ProcessorField::flags, builder);
  }
  ...
  Value loadProcessorData(Location loc, int64_t index, OpBuilder &builder) {
    // First check that the data field is valid.
    Value dataValidValue =
        loadFieldValue(loc, ProcessorField::data_valid, builder);
    Value indexI32Value = builder.create<LLVM::ConstantOp>(
        loc, IntegerType::get(builder.getContext(), 32),
        builder.getI32IntegerAttr(index));
    Value testBitValue =
        builder.create<LLVM::AndOp>(loc, dataValidValue, indexI32Value);
    Value zeroI32Value = builder.create<LLVM::ConstantOp>(
        loc, IntegerType::get(builder.getContext(), 32),
        builder.getI32IntegerAttr(0));
    Value isValidValue = builder.create<LLVM::ICmpOp>(
        loc, builder.getI1Type(), LLVM::ICmpPredicate::ne, testBitValue,
        zeroI32Value);

    // Load the value; it should always be in bounds but may be ignored if the
    ...
    // return isValid ? data[index] : 0;
    Value zeroI64Value = builder.create<LLVM::ConstantOp>(
        loc, IntegerType::get(builder.getContext(), 64),
        builder.getI64IntegerAttr(0));
    return builder.create<LLVM::SelectOp>(loc, isValidValue, dataValue,
                                          zeroI64Value);
  }
